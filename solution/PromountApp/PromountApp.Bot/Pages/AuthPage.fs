    ...
    interface IPage with
        member this.OnMassageHandler ctx command = async {
            ...
            | Start ->
                ...
                return Some (AuthPage(userState, inbox, AuthPageState.AwaitUser))
            | AwaitUser ->
                ...
                | Some { Data = Some callback } ->
                    ...
                    | s when s = signInOutput ->
                        ...
                        return Some (AuthPage(userState, inbox, AuthPageState.SignIn))
                    | s when s = signUpOutput ->
                        sendMessage userState.User.Id userState.Lang.RegisterInputTipText ctx
                        return Some (AuthPage(userState, inbox, AuthPageState.SignUp))
                    | _ -> return None
                | _ ->
                    return None
            | SignUp ->
                ...
                | Some { Text = Some name } ->
                    ...
                    | Success [advertiser] ->
                        ...
                        return Some (AdvertiserPage(userState, inbox, advertiser))
                    | Error code ->
                        sendMessage userState.User.Id (sprintf (Printf.StringFormat<_>(userState.Lang.ErrorText))
                                                           (code.ToString())) ctx
                        inbox.Post(Command.Start, ctx)
                        return Some (AuthPage(userState, inbox, AuthPageState.Start))
                    | _ ->
                        sendMessage userState.User.Id (sprintf (Printf.StringFormat<_>(userState.Lang.ErrorText))
                                                           "Internal Error") ctx
                        inbox.Post(Command.Start, ctx)
