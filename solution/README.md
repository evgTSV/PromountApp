...
Собирает графики с различных провайдеров (Loki, Prometheus), которые поддерживаются основными библиотеками `.NET` и `Open Telemetry`
...
| GC_PERSPECTIVE_TOKEN  | Ключ для доступа к Perspective API на Google Cloud                |
| TG_BOT_TOKEN          | Ключ для Bots Telegram API                                        |

Запуск производится с помощью `./compose.yaml` файла.

#### Описание эндпойнтов

Для описания точек используется `Swagger` (`/swagger/index.html`)
...
`Grafana` также используется для визуализации статистики кампаний по дням (дешборд `Stats`)
...
---
Бот позволяет рекламодателем управлять своими рекламными кампаниями, а также визуализировать статистику
Имя бота - `@PromountAppBot`
